...
import { 
  ...
  Gamepad, User, BookOpen, Calendar
} from "lucide-react";

export default function RobloxEmail() {
  const [loading, setLoading] = useState(false);
  const [search, setSearch] = useState("");

  const templates = [
    { id: 1, name: "Roblox Studio Onboarding", subject: "Start Your Roblox Journey!", type: "Onboarding" },
    { id: 2, name: "Roblox Game Design Update", subject: "Your Roblox Project Feedback", type: "Feedback" },
    { id: 3, name: "Roblox Advanced Scripting", subject: "Ready for Advanced Scripting?", type: "Migration" },
  ];
    ...
    <div className="space-y-8 pb-12">
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage specialized communications for Roblox courses.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 space-y-6">
            <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Send Quick Roblox Email</h3>
            ...
            <button className="w-full py-3 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2">
              <Send className="w-4 h-4" /> Send Roblox Email
            </button>
          </div>
        </div>
            ...
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex gap-3">
                  ...
                  <div>
                    <div className="text-[10px] font-bold text-slate-800">Sent to Vihaan S.</div>
